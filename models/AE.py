...
class Autoencoder():
    ...
    def _build(self):
        ...
        # stack standard layers together - Conv2D
        ...
        # reshape the vector into a TENSOR that can be fed as input into the first
        # convolutional transpose layer
        x = Reshape(shape_before_flattening)(x)

        # stack standard layers together - ConvTranspose2D
        for i in range(self.n_layers_decoder):
            ...
            x = conv_t_layer(x)
            # up until the last layer apply the standard pattern
            if i < self.n_layers_decoder - 1:
                ...
                if self.use_dropout:
                    x = Dropout(rate = 0.25)(x)
            # if it is the last layer in the DECODER
            else:
